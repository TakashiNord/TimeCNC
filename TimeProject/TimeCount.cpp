...
void TTimeCounter::start(char *file,int A, int  maxf)
{
        f=0;
        TIME=0;
        TIME1=0;
...
*/
int TTimeCounter::read_word(char *s,char w,char *sw)
{   char *word,sym;
    int  n;
...
/* Эта функция преобразует подачу формата up-файла
   в целое число f */
int TTimeCounter::feed_convert1(int ff)
{
int x,y,f;
...
float TTimeCounter::analiz(void)
{
...
/* Эта функция вставляет в очередь Р перемещение и подачу
   кадра kadr */
void TTimeCounter::push(char *kadr)
{
float x,y,z;
int ff;
